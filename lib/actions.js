...
Actions.prototype.setup = function () {
	if(this.config.containersConfig === true) {
		...
		console.log('Deploy to container-like sever!!'.bold);
		var self = this;
		...
		request({
			...
			if(self.config.servers[0].sshOptions) {
				self.config.servers[0].sshOptions.port = port;
			} else {
				self.config.servers[0].sshOptions = { port: port };
			}
			self.config.servers[0].password = containerResponse._password;
			var configFile = path.resolve(self.cwd, self.settingsFileName);
			exec('sed -i -e "s/\/\/{{PASSWORD}}/\"password\":\"' + containerResponse._password + '\",/g" -e "s/\/\/{{PASSWORD}}/\"sshOptions\":{\"port\":' + port + '},/g" ' + configFile);
			request({
				uri: taskUrl,
				method: 'POST',
				json: {
